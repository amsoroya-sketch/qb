  ...
  public static class QueryBuilder
  {
    ...
    public static IQueryable<T> BuildQuery<T>(this IQueryable<T> query, HashSet<string> includes) where T : class
    {
      foreach (var includePath in includes)
      {
        ...
        LambdaExpression previousLambda = null;
        ...
        for (int i = 0; i < pathParts.Length; i++)
        {
          ...
          string qry = currentQuery.ToString();
    ...
    /// <summary>
    /// Applies ordering based on custom [DLINQOrderby] attributes.
    /// </summary>
    ...
    /// <summary>
    /// Applies EF Core Include with optional filtering and ordering.
    /// </summary>
    private static IQueryable ApplyInclude(IQueryable query, Type queryEntityType, PropertyInfo relatedPropInfo)
    {
      ...
      var parentTypeLambda = Expression.Lambda(propertyAccess, queryEntityParam);
    ...
    { 
      ...
      foreach (var fields in selectFeilds)
      {
        ...
        projectionParts.Add($"{projectionParts} AS {alias}");
