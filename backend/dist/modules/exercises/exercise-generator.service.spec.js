...
const exercise_generator_service_1 = require("./exercise-generator.service");
...
describe('ExerciseGeneratorService', () => {
    ...
    describe('generateVerbAspectExercise', () => {
        ...
        it('should throw error if aspect data is missing', async () => {
            ...
            mockPrismaService.quranVerse.findUnique.mockResolvedValue(mockVerse);
            await expect(service.generateVerbAspectExercise(1, 1, 1)).rejects.toThrow('No aspect data available');
        });
    ...
    describe('generateNounCaseExercise', () => {
        it('should generate a nominative noun case exercise', async () => {
            ...
            expect(result.type).toBe(client_1.ExerciseType.NOUN_DECLENSION);
        ...
        it('should throw error if word is not a noun/adjective', async () => {
            ...
            mockPrismaService.quranVerse.findUnique.mockResolvedValue(mockVerse);
            await expect(service.generateNounCaseExercise(1, 1, 1)).rejects.toThrow('not a noun/adjective');
        });
    });
    describe('generateRootExtractionExercise', () => {
        it('should generate root extraction exercise with distractors', async () => {
            ...
            expect(result.type).toBe(client_1.ExerciseType.ROOT_EXTRACTION);
            expect(result.question).toContain('root');
            expect(result.options).toHaveLength(4);
            expect(result.options.map((o) => o.value)).toContain('كتب');
        ...
        it('should generate unique distractors', async () => {
            ...
            const rootValues = result.options.map((o) => o.value);
        ...
        it('should throw error if root data is missing', async () => {
            ...
            mockPrismaService.quranVerse.findUnique.mockResolvedValue(mockVerse);
            await expect(service.generateRootExtractionExercise(1, 1, 1)).rejects.toThrow('No root data available');
        });
    });
    describe('generateMorphemeIdentificationExercise', () => {
        it('should generate morpheme identification exercise with PREFIX|STEM|SUFFIX', async () => {
            ...
            expect(result.type).toBe(client_1.ExerciseType.MORPHEME_IDENTIFICATION);
        ...
        it('should throw error if word has simple structure (no affixes)', async () => {
            ...
            mockPrismaService.quranVerse.findUnique.mockResolvedValue(mockVerseWithWord);
            await expect(service.generateMorphemeIdentificationExercise(1, 1, 1)).rejects.toThrow('has no affixes');
        });
        it('should throw error if rawFeatures is missing', async () => {
            ...
            mockPrismaService.quranVerse.findUnique.mockResolvedValue(mockVerseWithWord);
            await expect(service.generateMorphemeIdentificationExercise(1, 1, 1)).rejects.toThrow('No morpheme data available');
        });
    });
    describe('generateSentenceTypeExercise', () => {
        it('should identify verbal sentence (starts with verb)', async () => {
            ...
            expect(result.type).toBe(client_1.ExerciseType.SENTENCE_TYPE);
        ...
        it('should throw error if verse not found', async () => {
            ...
            await expect(service.generateSentenceTypeExercise(1, 999)).rejects.toThrow('Verse 1:999 not found');
        });
        it('should throw error if verse has no words', async () => {
            ...
            mockPrismaService.quranVerse.findUnique.mockResolvedValue(mockVerse);
            await expect(service.generateSentenceTypeExercise(1, 1)).rejects.toThrow('No words found for verse');
        });
    });
    describe('generateSyntacticRoleExercise', () => {
        it('should generate syntactic role exercise for subject', async () => {
            ...
            expect(result.type).toBe(client_1.ExerciseType.SYNTACTIC_ROLE);
            expect(result.question).toContain('syntactic role');
            expect(result.correctAnswer).toBe('subject');
            expect(result.options.map((o) => o.value)).toContain('subject');
            expect(result.options.map((o) => o.value)).toContain('object');
            expect(result.options.map((o) => o.value)).toContain('predicate');
        });
        it('should throw error if syntactic role is missing', async () => {
            ...
            mockPrismaService.quranVerse.findUnique.mockResolvedValue(mockVerse);
            await expect(service.generateSyntacticRoleExercise(1, 1, 1)).rejects.toThrow('No syntactic role data available');
        });
    });
    describe('generateAgreementCheckingExercise', () => {
        it('should generate agreement exercise with matching properties', async () => {
            ...
            expect(result.type).toBe(client_1.ExerciseType.AGREEMENT_CHECKING);
        ...
        it('should throw error if no related word found for agreement', async () => {
            ...
            mockPrismaService.quranVerse.findUnique
                .mockResolvedValueOnce(mockVerse)
                .mockResolvedValueOnce(mockVerse);
            await expect(service.generateAgreementCheckingExercise(1, 1, 1)).rejects.toThrow('No related word found');
        });
        it('should throw error if no agreement properties found', async () => {
            ...
            mockPrismaService.quranVerse.findUnique
                .mockResolvedValueOnce(mockVerse)
                .mockResolvedValueOnce(mockVerse);
            await expect(service.generateAgreementCheckingExercise(1, 1, 1)).rejects.toThrow('No agreement properties found');
        });
    });
    describe('Helper Methods', () => {
        describe('parseMorphemes', () => {
            it('should parse PREFIX|STEM|SUFFIX format', () => {
                const result = service['parseMorphemes']('PREFIX | w:CONJ+ | STEM | POS:N | SUFFIX | CASE:NOM');
        ...
        describe('generateRootDistractors', () => {
            it('should generate 3 unique distractor roots', async () => {
                ...
                expect(distractors).not.toContain('كتب');
...
//# sourceMappingURL=exercise-generator.service.spec.js.map