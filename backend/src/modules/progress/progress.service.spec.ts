...
describe('ProgressService', () => {
  ...
  describe('getUserProgress', () => {
    it('should return user progress with calculated fields', async () => {
      const mockProgress = {
        ...
        totalTimeSpent: 3600,
        averageAccuracy: new Prisma.Decimal(85.5),
        createdAt: new Date(),
        updatedAt: new Date(),
      };
    ...
    it('should throw NotFoundException if user progress not found', async () => {
      ...
      await expect(service.getUserProgress('user-1')).rejects.toThrow(NotFoundException);
    ...
    it('should calculate level progress correctly', async () => {
      const mockProgress = {
        id: 'progress-1',
        userId: 'user-1',
        currentXP: 100, // 100 XP at level 1 (0-200)
        currentLevel: 1,
        currentStreak: 0,
        longestStreak: 0,
        lastActiveDate: null,
      ...
      // Level 1 requires 0 XP, Level 2 requires 200 XP
      // Progress should be 50% (100/200)
  ...
  describe('addXP', () => {
    ...
    it('should add XP and trigger level up', async () => {
      const mockProgress = {
        id: 'progress-1',
        userId: 'user-1',
        currentXP: 150, // 150 XP at level 1
        currentLevel: 1,
        currentStreak: 0,
        longestStreak: 0,
        lastActiveDate: null,
      ...
      // Adding 100 XP = 250 total, which is > 200 (level 2 threshold)
  ...
  describe('updateStreak', () => {
    ...
    it('should throw NotFoundException if user progress not found', async () => {
      ...
      await expect(service.updateStreak('user-1')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('getLessonProgress', () => {
    it('should return user lesson progress', async () => {
      ...
      expect(mockPrismaService.userLessonProgress.findMany).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { userId: 'user-1' },
        }),
      );
  ...
  describe('getDashboardStats', () => {
    it('should return comprehensive dashboard statistics', async () => {
      ...
      mockPrismaService.userExercise.count
        .mockResolvedValueOnce(17) // correctExercises (first call in getDashboardStats)
        .mockResolvedValueOnce(20) // totalExercises (second call)
        .mockResolvedValueOnce(17); // correctExercises (third call)
  ...
  describe('getAnalytics', () => {
    ...
    it('should return XP history for past 30 days', async () => {
      ...
      expect(result.xpHistory.length).toBe(31); // 30 days + today
    ...
    it('should calculate weekly stats comparison', async () => {
      ...
      mockPrismaService.userExercise.findMany
        .mockResolvedValueOnce([]) // First call for 30-day history
        .mockResolvedValueOnce([{ xpEarned: 50 }, { xpEarned: 50 }]) // Current week
        .mockResolvedValueOnce([{ xpEarned: 25 }]); // Last week
      mockPrismaService.userLessonProgress.findMany.mockResolvedValue([]);
      mockPrismaService.userLessonProgress.count
        .mockResolvedValueOnce(3) // Current week lessons
        .mockResolvedValueOnce(1); // Last week lessons
  ...
  describe('getTopicMastery', () => {
    it('should calculate accuracy per topic correctly', async () => {
      ...
      expect(result.topics.length).toBe(2); // Two different topics
  ...
  describe('getActivityCalendar', () => {
    ...
    it('should calculate activity levels correctly', async () => {
      ...
      mockPrismaService.userProgress.findUnique.mockResolvedValue(mockProgress);
      mockPrismaService.userExercise.findMany.mockResolvedValue([
        { completedAt: today, xpEarned: 200, timeSpent: 120, isCorrect: true }, // High
        { completedAt: yesterday, xpEarned: 100, timeSpent: 60, isCorrect: true }, // Medium
        { completedAt: twoDaysAgo, xpEarned: 30, timeSpent: 30, isCorrect: true }, // Low
      ]);
      mockPrismaService.userLessonProgress.findMany.mockResolvedValue([]);

      const result = await service.getActivityCalendar('user-1', 7);

      const todayActivity = result.calendar.find(
        (d) => d.date === today.toISOString().split('T')[0],
      );
      const yesterdayActivity = result.calendar.find(
        (d) => d.date === yesterday.toISOString().split('T')[0],
      );
      const twoDaysAgoActivity = result.calendar.find(
        (d) => d.date === twoDaysAgo.toISOString().split('T')[0],
      );
    ...
    it('should default to 365 days if invalid parameter', async () => {
      ...
      const result = await service.getActivityCalendar('user-1', 999); // Invalid: > 730
    ...
    it('should convert time to minutes correctly', async () => {
      ...
      mockPrismaService.userProgress.findUnique.mockResolvedValue(mockProgress);
      mockPrismaService.userExercise.findMany.mockResolvedValue([
        { completedAt: new Date(), xpEarned: 50, timeSpent: 120, isCorrect: true }, // 120 seconds = 2 minutes
      ]);
      mockPrismaService.userLessonProgress.findMany.mockResolvedValue([]);
      ...
      expect(todayActivity?.timeSpent).toBe(2); // Should be in minutes
    });
  });
});
