...
describe('TokenBlacklistService', () => {
  ...
  describe('blacklistToken', () => {
    ...
    it('should hash token before storing', async () => {
      ...
      expect(callArg).not.toContain('sensitive-token'); // Should not contain raw token
      expect(callArg).toContain('blacklist:token:'); // Should have prefix
      expect(callArg).toMatch(/^blacklist:token:[a-f0-9]{64}$/); // Should be SHA-256 hash
    });
    ...
    it('should use TTL matching token natural expiration', async () => {
      ...
      expect(redis.setex).toHaveBeenCalledWith(
        expect.any(String),
        7200, // Should match the expiresIn parameter
        'blacklisted',
      );
    });
  ...
  describe('blacklistAllUserTokens', () => {
    ...
    it('should use custom expiration when provided', async () => {
      ...
      await service.blacklistAllUserTokens('user-123', 86400); // 1 day
  ...
  describe('getBlacklistStats', () => {
    it('should return blacklist statistics', async () => {
      redis.keys
        ...
        .mockResolvedValueOnce(['blacklist:user:user1', 'blacklist:user:user2']); // User blacklist
    ...
    it('should return zero counts when no blacklisted entries', async () => {
      redis.keys
        .mockResolvedValueOnce([]) // No token blacklist
        .mockResolvedValueOnce([]); // No user blacklist
  ...
  describe('key generation', () => {
    ...
    it('should use SHA-256 hash for token keys', async () => {
      ...
      expect(hashPart).toMatch(/^[a-f0-9]{64}$/); // SHA-256 = 64 hex chars
    });
