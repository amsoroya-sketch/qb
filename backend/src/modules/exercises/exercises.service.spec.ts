...
describe('ExercisesService', () => {
  ...
  describe('submit', () => {
    it('should submit correct answer and award XP', async () => {
      ...
      expect(result.timeBonus).toBe(0); // No time bonus (> 30s)
    ...
    it('should award time bonus for fast completion', async () => {
      ...
      expect(result.timeBonus).toBe(2); // 20% of 10 = 2
      expect(result.xpEarned).toBe(12); // 10 + 2
    });
    ...
    it('should handle fill-in-blank with Arabic normalization', async () => {
      ...
      const exerciseId = 'exercise-1';
      const submitDto = {
        userAnswer: 'كِتَابٌ', // With diacritics
        timeSpent: 30,
      };
      ...
      const mockExercise = {
        id: exerciseId,
        type: 'FILL_IN_BLANK',
        correctAnswer: 'كتاب', // Without diacritics
        xpReward: 15,
      };
      ...
      expect(result.isCorrect).toBe(true); // Should match after normalization
    });
    ...
    it('should increment attempt number for repeat attempts', async () => {
      ...
      mockPrismaService.exercise.findUnique.mockResolvedValue(mockExercise);
      mockPrismaService.userExercise.count.mockResolvedValue(2); // 2 previous attempts
      ...
      expect(mockPrismaService.userExercise.create).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  describe('getExerciseStats', () => {
    it('should calculate exercise statistics', async () => {
      ...
      expect(result.accuracy).toBe(50); // 2/4 = 50%
      expect(result.avgTimeSpent).toBe(45); // (30+40+50+60)/4 = 45
    });
    ...
    it('should round accuracy and average time correctly', async () => {
      ...
      expect(result.accuracy).toBe(33.3); // 1/3 = 33.33% rounded to 1 decimal
      expect(result.avgTimeSpent).toBe(44); // (33+44+55)/3 = 44
    });
  });
});
