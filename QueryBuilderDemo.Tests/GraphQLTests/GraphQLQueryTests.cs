...
namespace QueryBuilderDemo.Tests.GraphQLTests
{
    /// <summary>
            ...
            var query = @"
{
    organisations(first: 10) {
        nodes {
            ...
            Assert.IsTrue(departments.Count > 0, "Organisation should have departments");

            // Verify department data is returned correctly
            var firstDept = departments[0];
            Assert.IsTrue(firstDept.TryGetProperty("name", out var deptName), "Department should have a name");
            Assert.IsNotNull(deptName.GetString());
        }
